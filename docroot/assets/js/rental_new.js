...
var d_getYear_format;
var d_getMonth_format;
var d_getDate_format;
var d_youbi;

// お受け取り希望日の日付取得処理
function deliveryDate(start_day_date_pick,key) {
  d_youbi_array = ["日","月","火","水","木","金","土"]; // 曜日表記を設定
  var start_day_date_pick_comp = new Date(start_day_date_pick);
  d_day_set = start_day_date_pick_comp.setDate(start_day_date_pick_comp.getDate() - key);
  d_day_date = new Date(d_day_set);
  d_day_date_from = formatDate(d_day_date);
  d_day = new Date(d_day_date_from);
  d_getYear_format = d_day.getFullYear();
  d_getMonth = d_day.getMonth() + 1;
  d_getMonth_format = ( '000' + d_getMonth ).slice( -2 );
  d_getDate = d_day.getDate();
  d_getDate_format = ( '000' + d_getDate ).slice( -2 );
  d_youbi = d_youbi_array[d_day.getDay()];
}
...
$('#airport-attention').hide();
// 宅配受取を選択した場合の処理
function selectDeivery(target) {
  ...
  $('input#zipcode_1').val('');
  $('input#zipcode_2').val('');
  $select_airport.val("空港を選択");
  $select_store.val("受取場所を選択");
  // 合計金額内の表示設定
...
// 空港受取を選択した場合の処理
function selectAirport(target) {
  $('.flex-delivery-date-area').hide();
  $('.flex-delivery-date-area .delivery-date-btn:not(".delivery-date-area-1")').hide();
  $('.select-place-caution').hide();
  $('#delivery-attention').hide();
  $('#airport-attention').show();


  // カレンダー（利用開始日）を選択していたら取得する
  var val_from = $('input[name="rental_start_date"]').val();
  var day_from;
  if(val_from){
    day_from = new Date(val_from);
  }
  ...
  // 現在の時間が配送が翌日になる時間より前かどうかで許容できる配送日数を分岐する
  var now_hour = new Date().getHours();
  var day_difference;
  if ( 0 <= now_hour && now_hour < delivery_switching ){
    day_difference = (day_from - nowTime) / 86400000 - 1;
  }else{
    day_difference = (day_from - nowTime) / 86400000 - 2;
  }

  // 選択した空港にある受取場所のみ表示
  $('#select-airport option').each(function(){
    // 配送にかかる日数取得
    var delivery_days = target.data('delivery-days');
    var wrap = target.parents().attr("class");

    if(day_difference < delivery_days){
      // optionタグをspanで囲い非表示にする
      if(wrap !== "wrap"){
        target.wrap("<span class='wrap'>");
      }
      $('.select-place-caution').show();
    }else{
      // spanタグ削除
      if(wrap === "wrap"){
        target.unwrap();
...
// 宅配受取を選択した場合（Enterキー押下時）
$('.js-input-radio-change.delivery').keydown(function(event) {
  if( event.keyCode == 13 ) { // 13：EnterキーのkeyCode
    const target = $(this).children('.relief-item-select-delivery');
    selectDeivery(target);
  }
});
// 空港受取を選択した場合（クリック時）
...
// 空港受取を選択した場合（Enterキー押下時）
$('.js-input-radio-change.airport').keydown(function(event) {
  if( event.keyCode == 13 ) { // 13：EnterキーのkeyCode
    const target = $(this).children('.relief-item-select-airport');
...
//受取場所のコンビニが1つだけの空港の場合ポップアップを非表示
if (window.location.pathname.includes("/entry/overseas/confirm")) {
  $('#select-store').on('change', function() {
    ...
    if(select_airport == '函館空港' || select_airport == '中部国際空港（名古屋）' || select_airport == '福岡空港') {
      $('.airport-edit-btn').hide();
    }
  });
}
// 空港・受け取り場所選択切り替え時
...
// 受取方法を選択の表示切替 end //////////////////////////////////////////////////////////////////////////////////////////////////
// お受け取り希望日を選択の表示切替 start //////////////////////////////////////////////////////////////////////////////////////////////////
$(".delivery-date-btn .topping-2-select-area-relief-option").click(function() {
  $(".delivery-date-btn .topping-2-select-area-relief-option").removeClass('selected');
  $(this).addClass('selected');
});
$(".delivery-time-btn .topping-2-select-area-relief-option").click(function() {
  $(".delivery-time-btn .topping-2-select-area-relief-option").removeClass('selected');
  $(this).addClass('selected');
});
$(".relief-item-select-delivery-day").click(function() {
  $(this).siblings("label").find("input").prop('checked', true).change();
});
$(".relief-item-select-delivery-time").click(function() {
  $(this).siblings("label").find("input").prop('checked', true).change();
});
// お受け取り希望日を選択の表示切替 end //////////////////////////////////////////////////////////////////////////////////////////////////
...
function open_period_form(){
  // 郵便番号以外のフォーム表示
  $('.hidden-block').removeClass('overseas-form-active');
}

// 郵便番号以下のフォームを閉じる
function close_period_form(){
  // 郵便番号以外のフォーム非表示
  $('.hidden-block').addClass('overseas-form-active');
}
// ユーザビリティ関連 end
...
$(function () {
  ...
  $("#search_btn").click(function () {
    ...
    var val_from_first = $('input[name="rental_start_date"]').val();
    // 利用開始日の選択中の日付
    var day_from_first = new Date(val_from_first);
    // 最低利用開始日
    var nowTimeMin_first = new Date(nowTimeMin);

    // 選択できる最短の渡航日も変更する
    $('input[name="rental_start_date"]').datepicker('setStartDate', delivery_info.set_start_date);
    if(day_from_first < nowTimeMin_first){
    ...
    if(option_from_area_comp > option_to_comp){
      ...
      // 事前配送オプションの最も早い選択可能日（利用不可）セッティング
      prior_delivery_date_start = false;
    }else if(option_from_date < option_from_area_comp){
      flg_delivery_day = true;
      // 事前配送オプションの最も早い選択可能日（郵便番号による判別）セッティング
      prior_delivery_date_start = option_from_area_comp;
    }else{
      flg_delivery_day = true;
      // 事前配送オプションの最も早い選択可能日（渡航日-6日）セッティング
      prior_delivery_date_start = option_from_comp;
    }
    // 事前配送オプションの最も遅い選択可能日（渡航日-2日）セッティング
    prior_delivery_date_end = option_to_comp;
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.status == 200 && res.results != null) {
          ...
          //送信ボタンを活性化
          $('#rental_entry_button').removeClass("form-next-button-disabled");
          if(prefectures.includes(result.address1) || delivery_days >= 4){
            ...
            if(prior_delivery_date_start){

              var general_priorStart = (( start_day_date_pick - prior_delivery_date_start ) / 86400000) + 1;

              if(start_day_date_pick){
                if(general_priorStart >= 2){
                  $('.delivery-date-area-2').show();
                  deliveryDate(start_day_date_pick,1);
                  $('.date-btn-2').html(d_getMonth_format + '/' + d_getDate_format + '（' + d_youbi + '）');
                  $('#delivery-date-2').val(d_getYear_format + '/' + d_getMonth_format + '/' + d_getDate_format);
                  $('#delivery-date-2').data('times', '1');
                }
                if(general_priorStart >= 3){
                  $('.delivery-date-area-3').show();
                  deliveryDate(start_day_date_pick,2);
                  $('.date-btn-3').html(d_getMonth_format + '/' + d_getDate_format + '（' + d_youbi + '）');
                  $('#delivery-date-3').val(d_getYear_format + '/' + d_getMonth_format + '/' + d_getDate_format);
                  $('#delivery-date-3').data('times', '2');
                }
                if(general_priorStart >= 4){
                  $('.delivery-date-area-4').show();
                  deliveryDate(start_day_date_pick,3);
                  $('.date-btn-4').html(d_getMonth_format + '/' + d_getDate_format + '（' + d_youbi + '）');
                  $('#delivery-date-4').val(d_getYear_format + '/' + d_getMonth_format + '/' + d_getDate_format);
                  $('#delivery-date-4').data('times', '3');
                }
                if(general_priorStart >= 5){
                  $('.delivery-date-area-5').show();
                  deliveryDate(start_day_date_pick,4);
                  $('.date-btn-5').html(d_getMonth_format + '/' + d_getDate_format + '（' + d_youbi + '）');
                  $('#delivery-date-5').val(d_getYear_format + '/' + d_getMonth_format + '/' + d_getDate_format);
                  $('#delivery-date-5').data('times', '4');
                }
                if(general_priorStart >= 6){
                  $('.delivery-date-area-6').show();
                  deliveryDate(start_day_date_pick,5);
                  $('.date-btn-6').html(d_getMonth_format + '/' + d_getDate_format + '（' + d_youbi + '）');
                  $('#delivery-date-6').val(d_getYear_format + '/' + d_getMonth_format + '/' + d_getDate_format);
                  $('#delivery-date-6').data('times', '5');
                }
              }
            }

            var nowTimeMin_comp = new Date(nowTimeMin);
            nowTimeMin_comp.setHours(0, 0, 0, 0);
            var start_day_date_pick_comp = new Date(start_day_date_pick);
            var start_day_date_pick_short = start_day_date_pick_comp.setDate(start_day_date_pick_comp.getDate() + 3);

            if(start_day_date_pick_short < nowTimeMin_comp){
              $('.delivery-area-caution').show();
              $('.flex-delivery-date-area').hide();
            }else{
              $('.delivery-area-caution').hide();
              $('.flex-delivery-date-area').show();
            }

            if(!prior_delivery_date_start){
              var nowTimeMin_delivery = new Date(nowTimeMin);
              nowTimeMin_delivery = new Date(nowTimeMin_delivery.setDate(nowTimeMin_delivery.getDate() - 1));
              var getYear = nowTimeMin_delivery.getFullYear();
              var getMonth1 = nowTimeMin_delivery.getMonth() + 1;
              var getMonth = ( '000' + getMonth1 ).slice( -2 );
              var getDate1 = nowTimeMin_delivery.getDate();
              ...
              // form送信用（指定なしの日付をvalに格納する）
              var deliveryUsually = getYear + '/' + getMonth + '/' + getDate;
              $('#delivery-date-1').val(deliveryUsually);
            }
          }
        } else {
          // 郵便番号が見つからない場合
          ...
          $('.delivery-first-title-area').addClass("mypage-user-info-detail-hidden");
          $('.save_delivery').addClass("mypage-user-info-detail-hidden");
          $('#rental_entry_button').addClass("form-next-button-disabled");
        }
      },
      // システムエラーが起こっている場合の警報
...
$(function(){
  $("#zipcode_1,#zipcode_2").change(function(){
    ...
    $('.flex-delivery-date-area').hide(); // 初期化
    $('.flex-delivery-date-area .delivery-date-btn:not(".delivery-date-area-1")').hide(); // 初期化
    $(".delivery-area-caution").hide(); // 初期化
...
// 事前配送オプションの可否日付格納
var prior_delivery_date_start; // 事前配送オプションの最も早い選択可能日
var prior_delivery_date_end; // 事前配送オプションの最も遅い選択可能日
...
$(function(){
  ...
  // 現在の時間が配送が翌日になる時間より前かどうかで最短利用開始日までの日数を分岐する
  var now_hour = new Date().getHours();
  var day_difference;
  if ( 0 <= now_hour && now_hour < delivery_switching ){
    earliest_start_date = 2;
  }else{
    earliest_start_date = 3;
  }

  // 郵便番号未選択時はminDaysに最短利用開始日を設定する
  minDays = '+' + earliest_start_date + 'd';
  ...
  .on('changeDate', function (selected) {
    ...
    $('#select-airport option').each(function(){
      // 配送にかかる日数取得
      var delivery_days = $(this).data('delivery-days');
      var wrap = $(this).parents().attr("class");
...
$('input[name="country_plan"]').on('change', function(){
  ...
  $('select[name="_areas"]').val(country_plan_area);
  $('select[name="_countries"]').val(country_plan);
  overseasSchedule();
});
  ...
  if(val_from != "" && val_to != "" && val_from != " " && val_to != " "){
    ...
    if(tour_id && gCountriesForTour[summary_area]){
      ...
      if(tour_price_comma){
        ...
        $('.plan-total-price').html('<span class="summary-text-device-option-price">' + rental_price_all_comma + '</span><span class="plan-tax"> 円<span class="text-sm">（免税）</span></span>');
      }
      ...
      if(mid_params == 1){
        summary_countries_dataplans = summary_countries + '&ensp;' + summary_dataplans + '（価格.com特典）';
      }else{
        summary_countries_dataplans = summary_countries + '&ensp;' + summary_dataplans;
      }
    ...
    if(summary_op_anshin == '25'){
      ...
      $('.summary-group-option-1').html('<span class="name-pc-wide">' + summary_op_anshin_text + '</span> <span>' + summary_op_anshin_price + '</span>').addClass('option-show');
      ...
      $('.summary-group-option-1').html('').removeClass('option-show');
    }
    ...
    if($('#support_option_03').prop('checked')){
      ...
      summary_op_advance_delivery_price = '<span>' + op_advance_delivery_price + '</span> 円';
      //事前配送料金（500円）×日数
      summary_op_advance_delivery_price_day = op_advance_delivery_price * activeDayCountNum;

      //受け取り日入力前なら「〇日前」を非表示
      if(isNaN(activeDayCountNum)){
        $('.summary-group-option-2').html('<span>' + summary_op_advance_delivery_text + '</span><span class="your-price2-sp"><span class="summary-option-2">' + op_advance_delivery_price + ' 円<span class="space"></span></span></span>');
      }else{
        $('.summary-group-option-2').html('<span>' + summary_op_advance_delivery_text + '</span><span class="your-price2-sp"><span class="summary-option-2">' + summary_op_advance_delivery_price_day.toLocaleString() + ' 円<span class="space"></span></span></span>').addClass('option-show');
        flg_delivery = true;
      ...
      $(".delivery-mandatory").hide();
      $('.summary-group-option-2').html('').removeClass('option-show');
      flg_delivery = false;
      flg_CountNum = false;
    }
    ...
    summary_op_advance_delivery_text = '事前配送オプション';
    op_advance_delivery_price = 500;
    var times = $('[name="delivery-date"]:checked').data('times');
    //事前配送料金（500円）×日数
    summary_op_advance_delivery_price = op_advance_delivery_price * times;
    summary_op_advance_delivery_price_day = String(summary_op_advance_delivery_price).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');

    //受け取り日入力前なら「〇日前」を非表示
    if(times){      
      $('.summary-group-option-2').
      html('<span>' + summary_op_advance_delivery_text + '</span> <span class="your-price2-sp"><span>' + summary_op_advance_delivery_price_day + '</span>  円<span class="space"></span></span>').addClass('option-show');
      op_advance_delivery_price = summary_op_advance_delivery_price;
    }else{
      $('.summary-group-option-2').html('').removeClass('option-show');
      op_advance_delivery_price = 0;
    }
    ...
    if($('#support_option_04').prop('checked')){
      ...
      $('.summary-group-option-3').html('<span>' + summary_op_return_pack_text + '</span> <span class="your-price2-sp">' + summary_op_return_pack_price + '</span>').addClass('option-show');
      ...
      if(campaign_params &&
        ...
        $('.summary-group-option-3-cp').html('<span>' + op_return_pack_cp_text + '</span> <span class="your-price2-sp"><span class="color-red">' + summary_op_return_pack_cp_price + '</span>').addClass('option-show');
        $('.op_return_pack-cp').html('<span>' + op_return_pack_cp_text + '</span></span>').addClass('option-show');
      }
    }else{
      $('.summary-group-option-3').html('').removeClass('option-show');
      $('.summary-group-option-3-cp').html('').removeClass('option-show');
    ...
    if($('#support_option_05').prop('checked')){
      ...
      op_transit_price_comma_text = '<span class="summary-text-device-option-price">' + op_transit_price_comma + '</span><span class="plan-tax"> 円<span class="text-sm">（免税）</span></span>';
      $('.summary-group-option-4').html('<span class="transit-name transit-name-1">' + summary_op_transit_text + '</span> <span class="summary-option-2 your-price2-sp">' + summary_op_transit_price + '</span><span class="space"></span>').addClass('option-show');
      $('.option-transit-price').html(op_transit_price_comma_text);
    }else{
      $('.transit-column').hide();
      $('.summary-group-option-4').html('').removeClass('option-show');
      $('.option-transit-price').html('');
    }
    if(op_anshin_price_total == 0 && op_advance_delivery_price == "" && op_return_pack_price == "" && op_transit_price !== ""){
      $(".option-total-price").html('<span class="summary-text-device-option-price">' + 0 + '</span> 円<span class="space-tax"></span>');
      $('.transit-column').show();
      ...
      op_total_price_comma_text = '<span class="summary-text-device-option-price">' + op_total_price_comma + '</span><span class="tax"> 円<span class="text-sm">（税抜）</span></span>';
    ...
    $('.summary-group-send').html('<span>' + send_text + '</span><span class="your-price2-sp">' + summary_send_price + '<span class="space"></span></span>').addClass('option-show');
    $('.summary-group-send-cp').html('<span>' + send_cp_text + '</span><span class="your-price2-sp color-red">' + summary_send_cp_price + '<span class="space"></span></span>');
    ...
    if(send_price || send_cp_price){
      ...
      send_total_comma_text = '<span class="summary-text-device-option-price">' + send_total_comma + '</span> 円';

      $('.send-cp').html('<span class="plan-tax">' + send_cp_text + '</span>');
      $('#send-cp-area').show();
    ...
    $('.overseas-your-all-price').html('<span class="big-num">' + overseas_your_all_price_comma + '</span><span> 円（税込）</span>');

    // 配送予定日時（事前配送オプションなし）
    var start_day = $('#datepicker_start').val();
    var start_day_date = new Date(start_day);
    start_day_date.setDate(start_day_date.getDate() - 3); // 前日を求める
    start_day_date_pick = new Date(start_day_date); // 他関数共有用
    ...
    var deliveryDay = $('[name="delivery-date"]:checked').val();
    $('.overseas-delivery-day').html(deliveryDay);
    $('.overseas-time').html('指定無し');

    var deliveryTime = $('[name="delivery-date-time"]:checked').data("text");
    $('.overseas-time').html(deliveryTime);
...
$('.topping-2-select-area,#device_option_true,.form-item-delivery').on('click change', function(){
  overseasSchedule();
});
$('.entry-option-select').on('click', function(){
  overseasSchedule();
});
$('.input-sm,._areas.js-tabindex,._countries.js-tabindex,._dataplans,[name="delivery-date"],[name="delivery-date-time"], [name="plan_id_tour"]').on('change', function(){
  overseasSchedule();
});
...
$('.input-sm,._areas,._countries,._dataplans,#anshin_option .entry-option-select,#detail-more-tab').on('click change', function(){
  if($('.your-plan-area').hasClass('summary-close')){
    if (window.matchMedia("(max-width: 768px)").matches) {
      //画面横幅が768px以下のときの処理
      $('.your-plan-area').css('height', 85);
    } else {
      //画面横幅が769px以上のときの処理
      $('.your-plan-area').css('height', 150);
    };
  }else{
    if (window.matchMedia("(max-width: 768px)").matches) {
      //画面横幅が768px以下のときの処理
      var your_plan_height = $('#your_plan').height() + 10;
    } else {
      //画面横幅が769px以上のときの処理
      var your_plan_height = $('#your_plan').height();
    };
...
$(function() {
  $(window).on("scroll", function() {
    ...
    const footHeight =280;/*追従の高さ(PC)*/
    const footHeightSP = 280;/*追従の高さ(SP)*/
...
$(function() {
  // チェックボックスとアンケート必須
  $("input#overseas-leave-checkbox, .question-area1, .other-input-form, input#answer_32, input#answer_37, input#answer_40").on('click change', function(){
...
$('#received_place_01,#received_place_02,#select-airport,#select-store,#zipcode_1,#zipcode_2,input[name="travel_type"],.input-sm,.topping-2-select-area,#device_option_true,.form-item-delivery,._areas,._countries,._dataplans,.entry-option-select,.country-plan,input[name="plan_id_tour"],.form-user-select-sp,.relief-item-select-delivery-day,.relief-item-select-delivery-time').on('click change', function(){
  // STEP2ではreturnさせる
  ...
  if(window.location.pathname.includes("/select")){
    ...
    var airport = $('select[name="select-airport"]').val();
  }
  ...
  var date = $('input[name="delivery-date"]:checked').val();
  var time = $('input[name="delivery-date-time"]:checked').val();
  var store = $('select[name="select-store"]').val();
...
$(function(){
  ...
  if(window.location.pathname.includes("/select") || window.location.pathname.includes("/confirm")){
    ...
    var airport_in = $('select[name="select-airport"]');
    ...
    var time_in = $('input[name="delivery-date-time"][value="' + time + '"]');
    var store_in = $('select[name="select-store"]');
    ...
    $(window).on('load', function() {
      if(date){
        var date_in = $('input[name="delivery-date"][value="' + date + '"]');
        date_in.closest('.delivery-date-btn').find('.relief-item-select-delivery-day').click();
      }
    });

    if(time){
      time_in.closest('.delivery-time-btn').find('.relief-item-select-delivery-time').click();
    }

    if(airport){
      airport_in.val(airport).change();
    }

    if(store){
      store_in.val(store).change();
    }
...
$(function() {
  if(window.location.pathname.includes("/entry/overseas/select") || window.location.pathname.includes("/entry/overseas/confirm")){
    $('.date-btn').each(function(index, elem) {
      var txt = $(elem).text();
      if(txt == '指定無し') {
        $(this).text('指定なし');
      }
