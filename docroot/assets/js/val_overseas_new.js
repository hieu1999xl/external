(function() {
  ...
  $.extend($.validator.messages, {
    ...
    new_email_conf: "正しいメールアドレスの形式で入力して下さい",
  ...
  var received_place = $('input[name="received_place"]:checked').val();
  var received_place_01;
  var received_place_02;

  if(received_place == "0"){
    received_place_01 = true;
    received_place_02 = false;
  }else{
    received_place_01 = false;
    received_place_02 = true;
  }

  //入力項目の検証ルール定義
  var rules = {
    // Plan
    plan_per_day: {required: true},
    plan_id_tour: {required: true},
    _countries: {required: true},
    rental_start_date: {required: true},
    rental_end_date: {required: true},
    prior_delivery_date: {required: true},
    rental_zipcode_1: {required: true, minlength: 3},
    rental_zipcode_2: {required: true, minlength: 4},
    "delivery-date": {required: received_place_01},
    "delivery-date-time": {required: received_place_01},
    "select-airport": {required: received_place_02},
    "select-store": {required: received_place_02},
    ...
    new_email_conf: { equalTo: "[name=new_email]" },
    ...
    city: { required: true },
    // town: { required: true },//町名を任意項目にするためコメントアウト
    block: { required: true },
    room_number: { required: ruleRoomNumber },
    // 法人
    business_type: { required: true },
    company: { required: true, zenkaku: true },
    company_kana: { required: true, zenkaku: true, katakana:true },
    ...
    corp_tel2_1 : { digits: true, maxlength: 5, minlength: 2 },
    corp_tel2_2 : { digits: true, maxlength: 5, minlength: 4 },
    corp_tel2_3 : { digits: true, maxlength: 5, minlength: 4 },
  ...
  //入力項目ごとのエラーメッセージ定義　
  let messages;
  if (currentPathIs('/entry/overseas/select')) {
    messages = {
      "tel1_2": {
    ...
    messages = {
      ...
      device_option: "端末あんしんオプションの加入有無を選択してください。",
      tie: "契約期間の縛り有無を選択してください。",
      plan_per_day: "プランを選択してください。",
      plan_id_tour: "プランを選択してください。",
      _countries: "利用国を選択してください。",
      rental_start_date: "渡航開始日を選択してください。",
      rental_end_date: "渡航終了日を選択してください。",
      prior_delivery_date: "受け取り希望日を選択してください。",
      rental_zipcode_1: "郵便番号上3桁を入力してください。",
      rental_zipcode_2: "郵便番号下4桁を入力してください。",
      "delivery-date": "お受け取り希望日を選択してください。",
      "delivery-date-time": "お受け取り希望時間帯を選択してください。",
      "select-airport": "空港を選択してください。",
      "select-store": "受取場所を選択してください。",
    };
  }
  ...
  $(function() {
    var option = {
      ...
      onfocusout: function(element) {
        ...
        if(elemName === "device_option" || elemName === "plan_per_day" || elemName === "plan_id_tour" || elemName === "_countries" || elemName === "rental_start_date" || elemName === "rental_end_date" || elemName === "prior_delivery_date" || elemName === "rental_zipcode_1" || elemName === "rental_zipcode_2" || elemName === "delivery-date" || elemName === "delivery-date-time" || elemName === "select-airport" || elemName === "select-store"){
          return false;
        }
        if(currentVal !== undefined && currentVal !== null){
          ...
          if(elemName === 'inquiry'){
            ...
            elemName.indexOf("company_hp_url") > -1
          ){
        ...
        if(elemName.indexOf("cardno-box") > -1) {
      ...
      errorPlacement: function(error, element) {
        ...
        if (inputName === "plan_per_day") {
          error.appendTo($("#plan-id-error-message"));
        }
        if (inputName === "plan_id_tour") {
          error.appendTo($("#plan-id-error-message-tour"));
        }
        if (inputName === "_countries") {
          error.appendTo($("#plan-id-error-message-country"));
        }
        if (inputName === "rental_start_date") {
          error.appendTo($("#plan-id-error-message-datepicker_start"));
        }
        if (inputName === "rental_end_date") {
          error.appendTo($("#plan-id-error-message-datepicker_end"));
        }
        if (inputName === "prior_delivery_date") {
          error.appendTo($("#plan-id-error-message-delivery"));
        }
        if (inputName === "rental_zipcode_1") {
          error.appendTo($("#plan-id-error-message-rental_zipcode_1"));
        }
        if (inputName === "rental_zipcode_2") {
          error.appendTo($("#plan-id-error-message-rental_zipcode_2"));
        }
        if (inputName === "delivery-date") {
          error.appendTo($("#plan-id-error-message-delivery-date"));
        }
        if (inputName === "delivery-date-time") {
          error.appendTo($("#plan-id-error-message-delivery-date-time"));
        }
        if (inputName === "select-airport") {
          error.appendTo($("#plan-id-error-message-select-airport"));
        }
        if (inputName === "select-store") {
          error.appendTo($("#plan-id-error-message-select-store"));
        }
      ...
      highlight: function(element, errorClass) {
        ...
        var elemName = $(element).attr("name");
        if(elemName === "plan_per_day" || elemName === "plan_id_tour" || elemName === "device_option" || elemName === "tie" || elemName === "_countries" || elemName === "rental_start_date" || elemName === "rental_end_date" || elemName === "prior_delivery_date" || elemName === "rental_zipcode_1" || elemName === "rental_zipcode_2" || elemName === "delivery-date" || elemName === "delivery-date-time" || elemName === "select-airport" || elemName === "select-store"){
          var speed = 500;
          var target;
          // var t = $("div#device-option-error-message").text();
          if(elemName ===  'rental_start_date'){
            target = $("#plan-id-error-message-datepicker_start");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  'rental_end_date' && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-datepicker_end");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  'plan_id_tour' && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-tour");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  '_countries' && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-country");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "plan_per_day" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "rental_zipcode_1" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-rental_zipcode_1");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "rental_zipcode_2" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-rental_zipcode_2");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "delivery-date" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-delivery-date");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "delivery-date-time" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-delivery-date-time");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "select-airport" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-select-airport");
            $(this).prop("hasPlanError", true);
          }else if(elemName === "select-store" && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-select-store");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  'prior_delivery_date' && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message-delivery");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  'installment_payment' && $(this).prop("hasPlanError") === false){
            target = $("#plan-id-error-message");
            $(this).prop("hasPlanError", true);
          }else if(elemName ===  'tie' && $(this).prop("hasPlanError") === false){
