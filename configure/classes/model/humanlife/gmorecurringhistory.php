 ...
 * ユーザー決済テーブルのモデルクラス
 */
class Model_HumanLife_GmoRecurringHistory extends Model_CrudAbstract {
     ...
     */
    protected static $_table_name = 'gmo_recurring_history';
...
SQL;
     ...
     * @return int 更新件数
     */
    public function update_result($update_params) {
...
SQL;
     ...
     * @return array
     */
    public function get_gmo_recurring_history_by_gmo_member_id($gmo_member_id ,  array $select = ['*']){
    ...
    /**
     * gmo_request_idをキーにsettlementinfo情報を取得する
     * @param int   $gmo_request_id
     * @param array $select      取得するカラム名の配列
     * @return array
     */
    public function get_gmo_recurring_history_by_gmo_request_id($gmo_request_id ,  array $select = ['*']){
        $query = DB::select_array($select)
                   ->from(self::$_table_name)
                   ->where('gmo_request_id', $gmo_request_id);
        return $query->execute()->current();
    }
